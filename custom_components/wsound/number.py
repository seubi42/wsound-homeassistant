...
async def async_setup_entry(hass, entry, async_add_entities):
    async_add_entities(
        [
            WSoundNumber(hass, entry.entry_id, "Volume", "volume", VOLUME_MIN, VOLUME_MAX, VOLUME_DEFAULT),
            WSoundNumber(hass, entry.entry_id, "Fade", "fade", FADE_MIN, FADE_MAX, FADE_DEFAULT),
            WSoundNumber(hass, entry.entry_id, "Duration", "duration", DURATION_MIN, DURATION_MAX, DURATION_DEFAULT),
        ]
    )
...
class WSoundNumber(NumberEntity, RestoreEntity):
    ...
    def __init__(self, hass, entry_id: str, name: str, key: str,
                 min_v: int, max_v: int, default: int) -> None:
        self._hass = hass
